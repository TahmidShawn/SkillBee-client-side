...
const JobDetails = () => {
    ...
    const { deadline, minPrice, maxPrice, title, description, email } = useLoaderData()
    ...
    const handleBids = event => {
        ...
        const user_deadline = { deadline }
        const user_email = form.user_email.value
        const buyer_email = form.buyer_email.value
        const myBids = {
            price, user_deadline, user_email, buyer_email
        }
        console.log(myBids);
                ...
                <div>
                    <div className="max-w-5xl mx-auto shadow-md p-10 border-[6px] mt-10">
                        ...
                        <form onSubmit={handleBids}>
                            ...
                            <div className="relative z-0 w-full mb-6 group">
                                <input type="email" name="buyer_email" defaultValue={email} readOnly id="buyer_email" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
